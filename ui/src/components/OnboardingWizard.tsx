...
const DEFAULT_TASK_DESCRIPTION = `请先将自己配置为 CEO。使用这里的 CEO 角色设定：
[https://github.com/paperclipai/companies/blob/main/default/ceo/AGENTS.md](https://github.com/paperclipai/companies/blob/main/default/ceo/AGENTS.md)

请确保你已有 \`agents/ceo\` 目录，并下载该目录中的 AGENTS.md 以及同级的 HEARTBEAT.md、SOUL.md、TOOLS.md。
然后把该 AGENTS.md 的路径配置为你的智能体指令文件。

完成后，请再为自己招聘一个 Founding Engineer 智能体。`;
...
export function OnboardingWizard() {
  ...
  // Step 3
  const [taskTitle, setTaskTitle] = useState("创建你的 CEO HEARTBEAT.md");
  ...
  function reset() {
    ...
    setTaskTitle("创建你的 CEO HEARTBEAT.md");
  ...
  async function handleStep2Next() {
    ...
    try {
      if (adapterType === "opencode_local") {
        ...
        if (!selectedModelId) {
          setError("OpenCode 需要显式指定 provider/model 格式的模型。");
          return;
        }
        ...
        if (adapterModelsLoading || adapterModelsFetching) {
          setError("OpenCode 模型仍在加载，请稍后重试。");
          return;
        }
        ...
        if (!discoveredModels.some((entry) => entry.id === selectedModelId)) {
          setError(
            discoveredModels.length === 0
              ? "未发现 OpenCode 模型，请先运行 `opencode models` 并完成 provider 认证。"
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <h3 className="font-medium">创建你的首个智能体</h3>
                      <p className="text-xs text-muted-foreground">
                        选择该智能体的任务运行方式。
                      </p>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <HintIcon text="建议为智能体创建独立目录，用于保存记忆并保持结构清晰。请创建新目录并填写路径。" />
                        </div>
                    ...
                    <div className="space-y-2 rounded-md border border-border p-3">
                      ...
                      {shouldSuggestUnsetAnthropicApiKey && (
                        <div className="rounded-md border border-amber-300/60 bg-amber-50/40 px-2.5 py-2 space-y-2">
                          ...
                          >
                            {unsetAnthropicLoading ? "重试中..." : "清空 ANTHROPIC_API_KEY"}
                          </Button>
                        </div>
                      )}
                      ...
                      <div className="rounded-md border border-border/70 bg-muted/20 px-2.5 py-2 text-[11px] space-y-1.5">
                        ...
                        <p className="text-muted-foreground font-mono break-all">
                          {adapterType === "cursor"
                            ? `${effectiveAdapterCommand} -p --mode ask --output-format json \"请回复 hello。\"`
                            : adapterType === "codex_local"
                            ? `${effectiveAdapterCommand} exec --json -`
                            : adapterType === "opencode_local"
                              ? `${effectiveAdapterCommand} run --format json "返回 hello。"`
                            : `${effectiveAdapterCommand} --print - --output-format stream-json --verbose`}
                        </p>
                        <p className="text-muted-foreground">
                          提示词：{" "}
                          <span className="font-mono">请回复 hello。</span>
                        </p>
                        {adapterType === "cursor" || adapterType === "codex_local" || adapterType === "opencode_local" ? (
                          ...
                          <p className="text-muted-foreground">
                            若需要登录，请执行{" "}
                            <span className="font-mono">claude login</span> 后重试。
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {(adapterType === "http" || adapterType === "openclaw_gateway") && (
                    <div>
                      <label className="text-xs text-muted-foreground mb-1 block">
                        {adapterType === "openclaw_gateway" ? "网关 URL" : "回调 URL"}
                      </label>
                      <input
                        className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm font-mono outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50"
                ...
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        先给智能体一个小任务开始，例如修复 bug、研究问题或编写脚本。
                      </p>
                    </div>
                  </div>
                  <div>
