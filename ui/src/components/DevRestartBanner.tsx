      ...
      <div className="flex flex-col gap-3 px-3 py-2.5 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm">
            ...
            {changedAt ? ` · ${t("updated_at")} ${changedAt}` : ""}
          </p>
        ...
        <div className="flex shrink-0 items-center gap-2 text-xs font-medium">
          {devServer.waitingForIdle ? (
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
              ...
              <span>{t("Waiting for")} {devServer.activeRunCount} {devServer.activeRunCount === 1 ? t("live run") : t("live runs")} {t("to finish")}</span>
            </div>
          ) : devServer.autoRestartEnabled ? (
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
            ...
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
              <RotateCcw className="h-3.5 w-3.5" />
              <span>{t("Restart pnpm dev:once after the active work is safe to interrupt")}</span>
            </div>
          )}
        </div>
      </div>
