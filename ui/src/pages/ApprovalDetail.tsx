...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? "审批详情" },
    ]);
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "审批拒绝失败"),
  });
  ...
  if (!approval) return <p className="text-sm text-muted-foreground">审批记录不存在。</p>;
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("确认删除该未通过审批的智能体？该操作不可恢复。")) return;
