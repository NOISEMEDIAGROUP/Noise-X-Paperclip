...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="请先选择公司以查看成本。" />;
  }
              ...
              <div className="flex items-center justify-between">
                ...
                {data.summary.budgetCents > 0 && (
                  <p className="text-sm text-muted-foreground">
                    预算使用 {data.summary.utilizationPercent}%
                  </p>
                )}
              </div>
                      ...
                      >
                        ...
                        <div className="text-right shrink-0 ml-2">
                          ...
                          <span className="text-xs text-muted-foreground block">
                            输入 {formatTokens(row.inputTokens)} / 输出 {formatTokens(row.outputTokens)} Token
                          </span>
                          {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) && (
                            <span className="text-xs text-muted-foreground block">
                              {row.apiRunCount > 0 ? `API 运行: ${row.apiRunCount}` : null}
                              {row.apiRunCount > 0 && row.subscriptionRunCount > 0 ? " | " : null}
                              {row.subscriptionRunCount > 0
                                ? `订阅运行: ${row.subscriptionRunCount}（输入 ${formatTokens(row.subscriptionInputTokens)} / 输出 ${formatTokens(row.subscriptionOutputTokens)} Token）`
                                : null}
                            </span>
                          )}
                        </div>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                {data.byProject.length === 0 ? (
                  <p className="text-sm text-muted-foreground">暂无按项目归因的运行成本。</p>
                ) : (
                  <div className="space-y-2">
                    {data.byProject.map((row) => (
                      ...
                      >
                        <span className="truncate">
                          {row.projectName ?? row.projectId ?? "未归因"}
                        </span>
