...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  const isInvalidPrefix = scope === "invalid_company_prefix";
  const title = isInvalidPrefix
    ? t("notFound.companyNotFound.title")
    : t("notFound.pageNotFound.title");
  const description = isInvalidPrefix
    ? t("notFound.companyNotFound.description", { prefix: normalizedPrefix ?? "unknown" })
