 ...
 *   --yolo             skip permission prompts (dangerouslySkipPermissions)
 */
 ...
 */
function detectHermesHome(hermesCmd: string | undefined): string {
  try {
    if (hermesCmd && hermesCmd !== HERMES_CLI) {
      // Try to resolve HERMES_HOME from wrapper script
      const cmdPaths = [
      ...
      for (const cmdPath of cmdPaths) {
        if (existsSync(cmdPath)) {
          ...
          if (match?.[1]) {
            return match[1].replace(/\$HOME/g, homedir()).replace(/~/, homedir());
          }
        }
      }
    }
...
function detectCurrentModel(hermesCmd: string | undefined): {
  model: string | null;
...
function buildPrompt(ctx: BuildPromptContext, config: Record<string, unknown>): string {
...
function parseHermesOutput(
  ...
  const result: ReturnType<typeof parseHermesOutput> = {};
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // ── Inject AGENTS.md from workspace (like Claude's --append-system-prompt-file) ──
  ...
  if (agentName) {
    ...
    for (const name of candidates) {
      const agentsFile = resolve(resolvedCwd, "agents", name, "AGENTS.md");
      if (existsSync(agentsFile)) {
        agentsFileContent = readFileSync(agentsFile, "utf8").trim();
        break;
      }
    }
  }

  // ── Resolve task details (Paperclip does not always send title/body in wake context) ──
  const resolvedTaskId =
    cfgString(ctx.context.taskId) || cfgString(ctx.context.issueId);
  let resolvedTaskTitle =
    cfgString(ctx.context.taskTitle) || cfgString(ctx.context.issueTitle);
  let resolvedTaskBody =
    cfgString(ctx.context.taskBody) || cfgString(ctx.context.issueBody);
  ...
  if (resolvedTaskId && (!resolvedTaskTitle || !resolvedTaskBody)) {
    try {
      ...
      if (issueResp.ok) {
        ...
        resolvedTaskTitle = resolvedTaskTitle || cfgString(issue.title) || "";
        resolvedTaskBody = resolvedTaskBody || cfgString(issue.description) || "";
      }
    } catch (_) {
      // non-fatal: continue without title/body
    }
  }
  ...
  if (cfgBoolean(config.dangerouslySkipPermissions) === true) args.push("--yolo");

  // Session resume
  const VALID_SESSION_ID = /^\d{8}_\d{6}_[a-zA-Z0-9]+$/;
  const prevSessionId = cfgString(ctx.runtime.sessionParams?.sessionId);
  const validPrevSessionId =
    prevSessionId && VALID_SESSION_ID.test(prevSessionId) ? prevSessionId : undefined;
  ...
  // Ensure Hermes terminal tool uses the workspace as cwd.
  // Hermes terminal is stateless — each command starts in a fresh shell.
  // Setting TERMINAL_CWD tells the Hermes terminal tool to cd into this
  // directory before every command (tools/terminal_tool.py line ~485).
  ...
  if (validPrevSessionId) {
    await ctx.onLog("stdout", `[hermes] Resuming session: ${validPrevSessionId}\n`);
  }
  ...
  // If no valid session ID found, explicitly clear sessionParams so Paperclip
  // doesn't retry the same invalid/missing session on the next run.
  if (persistSession && parsed.sessionId && VALID_SESSION_ID.test(parsed.sessionId)) {
